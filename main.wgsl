...
fn fragment_main(fragData: VertexOut) -> @location(0) vec4f {
	var color: vec3f = mix((textureLoad(diffuseTexture, vec2i(fragData.uv), 0).xyz + fragData.overlayColor) * max(dot(fragData.normal, normalize(lightDir)), 0.3), vec3f(0.53,0.81,0.92), clamp(fragData.position.w/100, 0, 1));
